...
export function renderChangedFileList(
  ...
  const items = files
    .map((f) => {
      const kindClass = `os-change-kind-${f.change_kind}`;
      const selected = selectedPath === f.path ? " os-selected" : "";
      return `<button class="os-changed-file${selected}" data-path="${escapeHtml(f.path)}" data-testid="changed-file-item" data-kind="${f.change_kind}">
        <span class="os-change-kind ${kindClass}">${f.change_kind}</span>
...
export function renderFileDiff(diff: FileDiffPage): string {
  if (diff.hunks.length === 0) {
    return `<div class="os-file-diff os-empty" data-testid="file-diff" data-file-path="${escapeHtml(diff.file_path)}">No diff available</div>`;
  }
  ...
  const hunks = diff.hunks
    .map((hunk) => {
      const lines = hunk.lines
        .map((line) => {
          const typeClass = `os-diff-line-${line.type}`;
          const prefix = line.type === "addition" ? "+" : line.type === "deletion" ? "-" : " ";
          return `<div class="os-diff-line ${typeClass}" data-line-type="${line.type}"><span class="os-diff-prefix">${prefix}</span>${escapeHtml(line.line)}</div>`;
        })
        .join("");
    ...
    .join("");
  return `<div class="os-file-diff" data-testid="file-diff" data-file-path="${escapeHtml(diff.file_path)}">${header}${hunks}</div>`;
}
