...
export function renderValidationSummary(
  ...
  const statusClass = `os-validation-status-${summary.overall_status}`;
  const header = `<div class="os-validation-header" data-testid="validation-header">
    <span class="os-validation-status ${statusClass}">${summary.overall_status}</span>
    <span class="os-validation-timestamp">${summary.generated_at}</span>
  </div>`;
  ...
  return `<div class="os-validation-summary" data-testid="validation-summary" data-run-id="${summary.run_id}">${header}${commands}${evidence}</div>`;
}

function renderCommand(cmd: ValidationCommand): string {
  const statusClass = `os-validation-status-${cmd.status}`;
  return `<div class="os-validation-command" data-testid="validation-command" data-command-id="${cmd.command_id}">
    <div class="os-command-header">
      <span class="os-command-status ${statusClass}">${cmd.status}</span>
...
function renderEvidence(item: ValidationEvidenceItem): string {
  const statusClass = `os-validation-status-${item.status}`;
  return `<div class="os-validation-evidence-item" data-testid="validation-evidence-item" data-evidence-id="${item.evidence_id}">
    <div class="os-evidence-header">
      <span class="os-evidence-status ${statusClass}">${item.status}</span>
