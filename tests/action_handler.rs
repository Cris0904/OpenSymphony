...
async fn action_handler_returns_accepted_receipt_with_correlation_id() {
    ...
    let envelope = fixture_envelope("COE-255", IssueRuntimeState::Running, WorkerOutcome::Running);
...
async fn action_handler_rejects_rehydrate_on_running_issue() {
    ...
    let receipt = handler.dispatch(dispatch, &envelope).await;
    assert_eq!(receipt.status, ActionStatus::Rejected);
    assert!(receipt.reason.as_ref().unwrap().contains("Rehydrate is only available"));
}
...
async fn action_handler_accepts_rehydrate_on_completed_with_failed_outcome() {
    let journal = InMemoryEventJournal::new(1000, 64);
    let handler = ActionHandler::new(journal.clone());
    let envelope = fixture_envelope("COE-255", IssueRuntimeState::Completed, WorkerOutcome::Failed);
...
async fn action_handler_rejects_rehydrate_on_completed_with_running_outcome() {
    let journal = InMemoryEventJournal::new(1000, 64);
    let handler = ActionHandler::new(journal.clone());
    let envelope = fixture_envelope("COE-255", IssueRuntimeState::Completed, WorkerOutcome::Running);
...
async fn action_handler_publishes_accepted_event_to_journal() {
    ...
    assert_eq!(page.events[0].kind, EventKind::GatewayActionDispatched { action: "cancel".into() });
}
...
async fn action_handler_publishes_rejected_event_to_journal() {
    ...
    assert!(matches!(page.events[0].kind, EventKind::GatewayActionFailed { .. }));
}
...
async fn action_handler_rejects_duplicate_idempotency_key() {
    ...
    assert!(receipt2.reason.as_ref().unwrap().contains("duplicate idempotency key"));
}
...
async fn action_handler_returns_expected_followups_for_retry() {
    ...
    assert!(receipt.expected_followup.iter().any(|f| matches!(f, ExpectedFollowup::ActionCompletion)));
    assert!(receipt.expected_followup.iter().any(|f| matches!(f, ExpectedFollowup::RunLifecycle)));
}
...
async fn action_handler_comment_is_accepted_on_any_state() {
    ...
    for (state, outcome) in [
        ...
        let receipt = handler.dispatch(dispatch, &envelope).await;
        assert_eq!(receipt.status, ActionStatus::Accepted, "comment should be accepted in {:?}", state);
    }
}
